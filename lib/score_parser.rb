class ScoreParser
  ...
  ADJUSTED_SCORE_CMD=SCORE_BIN_PATH+"/runexample_adjusted.sh"
  ...
  def calculate_payoff(sample)
	...
	def calculate_adjusted_payoff(sample)
    check = check_file_clean(sample.full_filename) if sample.full_filename
    #puts check
    unless check.first == "0"
      sample.update_attribute(:clean, false)
    ...
    scores = read_adjusted_scores_from_file(sample.full_filename) if sample.full_filename
    #puts scores
    #puts scores[0]
    unless scores
      sample.update_attribute(:clean, false)
    ...
    three_player_adjusted_payoff = sample.three_player_adjusted_payoff
    unless three_player_adjusted_payoff
      three_player_adjusted_payoff = sample.build_three_player_adjusted_payoff      
      three_player_adjusted_payoff.three_player_profile = sample.profile
    end
    ...
    for i in 0..2
      ...
      payoff = payoffs[i] / 2  
      puts "strategy_#{player}_payoff=", payoff
      three_player_adjusted_payoff.send("strategy_#{player}_payoff=", payoff)
    end
    
    three_player_adjusted_payoff.save
    # sample.update_attribute(:clean, true)
    sample.three_player_payoff.update_attribute(:adjusted, true)
  end
  ...
  def read_scores_from_file(game_file)
	...
	def read_adjusted_scores_from_file(game_file)
    
    f = IO.popen("sh #{ADJUSTED_SCORE_CMD} -file #{game_file}")
