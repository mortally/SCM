...
//   private boolean isScratched = false;
  ...
  private void readHeader() throws IOException, ParseException {
    ...
    // The Coordinator. How should this be handled??? FIX THIS!!!
    ...
    while (reader.nextNode("participant", false)) {
      ...
      ParticipantInfo p =
	new ParticipantInfo(index,
			    reader.getAttribute("address"),
			    reader.getAttributeAsInt("id", -1),
			    reader.getAttribute("name", null),
			    reader.getAttributeAsInt("role"));
      if (index >= participants.length) {
	// Should check for ridiculous large indexes. FIX THIS!!!
	participants = (ParticipantInfo[])
	  ArrayUtils.setSize(participants, index + 10);
      }
      if (participants[index] != null) {
	throw new ParseException("participant " + index + " already set",
				 0); // Position? FIX THIS!!!
      }
    ...
    // Perhaps should verify that all participants has been specified? FIX THIS!!!
    if (participantCount < participants.length) {
      participants = (ParticipantInfo[])
	ArrayUtils.setSize(participants, participantCount);
    }
    reader.exitNode();
  }
...
// 	    System.out.println("======================================================================");
// 	    System.out.println("LARGE NODE (" + len + "): " + reader.getNodeName());
// 	    reader.printMessage();
