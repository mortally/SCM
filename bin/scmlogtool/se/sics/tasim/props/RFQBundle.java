 ...
 * Updated : $Date: 2005/04/01 12:12:46 $
 *           $Revision: 1.13 $
 */
...
public class RFQBundle implements Transportable, java.io.Serializable {
   ...
   * @param penalty the penalty for late deliveries or 0 for no penalty
   * @throws IllegalStateException if this bundle is locked
   */
  public synchronized void addRFQ(int rfqID, int productID,
				  int quantity, int reservePricePerUnit,
				  int dueDate, int penalty) {
    if (isLocked) {
      throw new IllegalStateException("locked");
    }
   ...
   *
   * @param rfqID the id of the RFQ
   * @return the index of the specified RFQ or -1 if the RFQ was not
   * found in this bundle
   */
  public int getIndexFor(int rfqID) {
    if ((rfqID < (startID + rfqCount))
	&& (rfqID >= startID)
   ...
   *
   * @param index the index of the RFQ in this bundle
  ...
  /**
   * Returns the total quantity of all products requested in all RFQs
   * in this bundle.
   */
  ...
  /**
   * Returns the penalty for late deliveries of the specified RFQ.
   * The penalty is specified as the total sum that is charged on late
   * deliveries.  Usually the penalty is charged day for day until the
   * delivery has been made or the order been voided.
   *
   * @param index the index of the RFQ in this bundle
  ...
  /**
   * Returns the reserve price per unit for the specified RFQ.  This
   * is the maximal price the sender of the request is willing to pay
   * per unit in the order.<p>
   *
   * This is optional information that the sender might supply.
   *
   * @param index the index of the RFQ in this bundle
   * @return the reserve price per unit or 0 if no reserve price has
   * been specified by the sender
   */
  ...
  public void read(TransportReader reader) throws ParseException {
	...
	reader.getAttributeAsInt(names[RESERVE_PRICE], 0);
      if (reservePricePerUnit == 0 && quantity > 0) {
	reservePricePerUnit =
	  reader.getAttributeAsInt("reservePrice", 0) / quantity;
      }
      // END OF BACKWARD COMPABILITY: REMOVE THIS!!!
      addRFQ(reader.getAttributeAsInt(names[RFQ_ID]),
