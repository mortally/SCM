...
public abstract class SimpleContent implements Transportable,
  ...
  private static final Logger log =
    Logger.getLogger(SimpleContent.class.getName());
  ...
  public int getAttributeAsInt(String name, int defaultValue) {
	...
	log.warning("attribute '" + name + "' has a non-integer value '"
		    + value + '\'');
      }
    }
    return defaultValue;
  ...
  private Object get(String name) {
    int index = ArrayUtils.keyValuesIndexOf(attributePairs, PARTS,
					    0, attributeCount * PARTS, name);
    return (index >= 0)
      ? attributePairs[index + VALUE]
      : null;
  }
  ...
  private void set(String name, Object value) {
	...
	attributePairs =
	  ArrayUtils.setSize(attributePairs, index + 10 * PARTS);
      }
  ...
  protected StringBuffer params(StringBuffer buf) {
    buf.append('[');
    if (attributeCount > 0) {
      buf.append(attributePairs[NAME]).append('=')
	.append(attributePairs[VALUE]);
      for (int i = PARTS, n = attributeCount * PARTS; i < n; i += PARTS) {
	buf.append(',')
	  .append(attributePairs[i + NAME]).append('=')
