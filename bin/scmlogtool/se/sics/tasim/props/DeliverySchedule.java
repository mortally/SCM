 ...
 * Updated : $Date: 2003/06/13 18:10:51 $
 *           $Revision: 1.9 $
 */
...
/**
 * <code>DeliverySchedule</code> contains information about deliveries
 * to make.
 *
...
public class DeliverySchedule implements Transportable, java.io.Serializable {
   ...
   * @param orderID the id of the order for which the delivery will be made
   * @param customer the address of the receiver of the delivery
   */
  public synchronized void addDelivery(int productID, int quantity,
	...
	ArrayUtils.setSize(deliveryReceiver, count + 20);
    }
  ...
  public String toString() {
	...
	sb.append(',').append(data[i * PARTS + j]);
      ...
      sb.append(',').append(deliveryReceiver[i])
	.append(']');
    }
  ...
  public void write(TransportWriter writer) {
    for (int i = 0, index = 0; i < count; i++, index += PARTS) {
      writer.node("delivery")
	.attr("order", data[index + ORDER_ID])
	.attr("product", data[index + PRODUCT_ID])
	.attr("quantity", data[index + QUANTITY])
	.attr("customer", deliveryReceiver[i])
	.endNode("delivery");
    }
  }
