 ...
 * Updated : $Date: 2003/07/01 17:34:12 $
 *           $Revision: 1.12 $
 */
...
/**
* <code>ComponentCatalog</code> contains information about which
* components that are available and which supplier(s) that are
* producing them. This message is typically sent in the beginning of a
* simulation/game.
 *
...
public class ComponentCatalog implements Transportable, java.io.Serializable {
  ...
  public synchronized void addSupplier(String supplier, int[] productID) {
    ...
    if (productID != null) {
      int supIndex =
	ArrayUtils.indexOf(supplierList, 0, supplierCount, supplier);
      if (supIndex < 0) {
	if (supplierCount == supplierList.length) {
	  supplierList = (String[])
	    ArrayUtils.setSize(supplierList, supplierCount + 10);
	}
	...
	suppliers[index] = (String[])
	  ArrayUtils.add(String.class, suppliers[index], supplier);
      }
    }
  }
  ...
  /**
   * Returns the suppliers for the product/component with the specified ID
   * Note: returns NULL if no supplier exists
   */
  ...
  public String toString() {
	...
	buf.append(',');
      ...
      buf.append('[').append(data[index + PRODUCT_ID]).append('=')
	.append(productNames[i])
	.append(',').append(data[index + BASE_PRICE]);
      if (supps != null) {
