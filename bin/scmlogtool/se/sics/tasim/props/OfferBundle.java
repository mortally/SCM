 ...
 * Updated : $Date: 2003/06/13 18:10:52 $
 *           $Revision: 1.8 $
 */
...
/**
 * <code>OfferBundle</code> contains an ordered set of offers.
 * Each offer is a response to an RFQ and contains:
 * <ul>
 ...
 * </ul>
 * Any other information such as product and penalty can be found in
 * the referred RFQ.
 *
...
public class OfferBundle implements Transportable, java.io.Serializable {
  ...
  public void addOffer(int offerID, RFQBundle rfqs, int rfqIndex,
		       int unitPrice) {
    addOffer(offerID, rfqs, rfqIndex, unitPrice,
	     rfqs.getDueDate(rfqIndex), rfqs.getQuantity(rfqIndex));
  }
  ...
  public void addOffer(int offerID, int rfqID, int unitPrice,
  ...
  public void read(TransportReader reader) throws ParseException {
    ...
    while (reader.nextNode("offer", false)) {
      add(reader.getAttributeAsInt("id"),
	  reader.getAttributeAsInt("rfq"),
	  reader.getAttributeAsInt("unitprice"),
	  reader.getAttributeAsInt("duedate"),
	  reader.getAttributeAsInt("quantity"));
    }
    if (lock) {
      lock();
    }
  ...
  public void write(TransportWriter writer) {
    ...
    for (int index = 0, n = count * PARTS; index < n; index += PARTS) {
      writer.node("offer")
	.attr("id", data[index + OFFER_ID])
	.attr("rfq", data[index + RFQ_ID])
	.attr("unitprice", data[index + UNIT_PRICE])
	.attr("duedate", data[index + DUEDATE])
	.attr("quantity", data[index + QUANTITY])
	.endNode("offer");
    }
  }
