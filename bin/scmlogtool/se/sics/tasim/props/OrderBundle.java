...
/**
 * <code>OrderBundle</code> contains an set of orders.
 * Each order contains:
 * <ul>
...
public class OrderBundle implements Transportable, java.io.Serializable {
  ...
  public String toString() {
    ...
    for (int index = 0, n = count * PARTS; index < n; index += PARTS) {
      ...
      buf.append('[').append(data[index + ORDER_ID]).append(',')
	.append(data[index + OFFER_ID]).append(']');
    }
  ...
  public void read(TransportReader reader) throws ParseException {
    ...
    while (reader.nextNode("order", false)) {
      addOrder(reader.getAttributeAsInt("id"),
	       reader.getAttributeAsInt("offer"));
    }
    if (lock) {
      lock();
    }
  ...
  public void write(TransportWriter writer) {
    ...
    for (int i = 0, index = 0; i < count; i++, index += PARTS) {
      writer.node("order")
	.attr("id", data[index + ORDER_ID])
	.attr("offer", data[index + OFFER_ID])
	.endNode("order");
    }
  }
