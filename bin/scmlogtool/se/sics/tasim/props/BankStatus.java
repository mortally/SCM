 ...
 * Updated : $Date: 2003/06/25 12:44:35 $
 *           $Revision: 1.13 $
 */
...
public class BankStatus implements Transportable, java.io.Serializable {
  ...
  public void addPenalty(String penaltyReceiver, int orderID, int amount) {
    if (penaltyReceivers == null) {
      ...
      penaltyReceivers = (String[])
	ArrayUtils.setSize(penaltyReceivers, newSize);
   ...
   *
   * @param index the index of the penalty
  ...
  /**
   * Returns the total amount claimed for all penalties specified in
   * this bank status.
   */
  ...
  public void addDeniedDelivery(String denier, int orderID) {
    if (deniedDeliveries == null) {
      ...
      deniedDeliveries =
	ArrayUtils.setSize(deniedDeliveries, deniedCount + 10);
      deniers = (String[])
	ArrayUtils.setSize(deniers, deniedCount + 10);
    }
  ...
  public void read(TransportReader reader) throws ParseException {
	...
	receiver = (receiver == null)
	  ? reader.getAttribute("receiver")
	  : reader.getAttribute("receiver", receiver);
	addPenalty(receiver,
		   reader.getAttributeAsInt("orderID"),
		   reader.getAttributeAsInt("amount"));
      } else if (reader.isNode("deniedDelivery")) {
	denier = (denier == null)
	  ? reader.getAttribute("denier")
	  : reader.getAttribute("denier", denier);
	addDeniedDelivery(denier,
			  reader.getAttributeAsInt("orderID"));
      }
    }
  }
  ...
  public void write(TransportWriter writer) {
	...
	writer.attr("receiver", receiver);
      ...
      writer.attr("orderID", penalties[index + ORDER_ID])
	.attr("amount", penalties[index + AMOUNT])
	.endNode("penalty");
    }
	...
	writer.attr("denier", denier);
      ...
      writer.attr("orderID", deniedDeliveries[i])
	.endNode("deniedDelivery");
    }
  }
