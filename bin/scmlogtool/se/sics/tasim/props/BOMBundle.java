 ...
 * Updated : $Date: 2003/07/31 13:28:34 $
 *           $Revision: 1.8 $
 */
 ...
 * that can be produced and which components that are needed for each
 * product. A BOMBundle is typically send in the beginning of a game/
 * simulation.
 *
...
public class BOMBundle implements Transportable, java.io.Serializable {
  ...
  public void addBOM(int productID, int cycles,
		     int[] components, int[] substitutes) {
    addBOM(productID, cycles, components, substitutes, null, 0);
  }

  public synchronized void addBOM(int productID, int cycles,
				  int[] components, int[] substitutes,
				  String productName, int basePrice) {
    if (isLocked) {
    ...
    if (componentLength + compLen + substLen + 2 >= componentData.length) {
      componentData = ArrayUtils.setSize(componentData,
					 componentLength
					 + compLen + substLen + 2
					 + 20);
    }
   ...
   * @return the index of the specified product or -1 if the product
   * was not found in this bundle
   */
  ...
  /**
   * Returns the number of assembly cycles required to produce the
   * product at position <code>index</code>.
   */
  ...
  /**
   * Returns the base price of the product at position
   * <code>index</code> or 0 if no base price is known.
   */
  ...
  /**
   * Returns the name of the component/product at position
   * <code>index</code>.
   */
   ...
   *
   * @param productID the id of the product
   * @return an array of component ids or <CODE>null</CODE> if the
  ...
  /**
   * Returns an array of productIDs that represent substitutes for the
   * specified productID or <CODE>null</CODE> if no such substitutes
   * exists.
   *
   * <p>Note: product substitutes are not used in TAC03 SCM.
   */
  public int[] getSubstitutesForProductID(int productID) {
    int index = getIndexFor(productID);
    return (index >= 0)
      ? getSubstitutes(index)
      : null;
  }

  /**
   * Returns an array of productIDs that represent substitutes for the
   * productID at position <code>index</code> or <CODE>null</CODE> if
   * no such substitutes exists.
   *
   * <p>Note: product substitutes are not used in TAC03 SCM.
   */
  ...
  public String toString() {
	...
	sb.append(',');
      ...
      sb.append(getProductName(i))
	.append('(').append(getProductID(i))
	.append(")={");
  ...
  public void write(TransportWriter writer) {
    ...
    for (int i = 0, index = 0; i < count; i++, index += PARTS) {
      ...
      writer.node("product")
	.attr("id", data[index + PRODUCTION_ID])
	.attr("cycles", data[index + CYCLES]);
      if (basePrice != 0) {
	...
	writer.node("component").attr("id", componentData[j])
	  .endNode("component");
      }
      for (int j = substIndex, m = substIndex + substNumber; j < m; j++) {
	writer.node("substitute").attr("id", componentData[j])
	  .endNode("substitute");
      }
