 ...
 * Updated : $Date: 2004/02/17 18:39:18 $
 *           $Revision: 1.9 $
 */
...
/**
 * <code>InventoryStatus</code> contains information about the
 * quantity of products in an inventory.
 *
...
public class InventoryStatus implements Transportable, java.io.Serializable {
   ...
   *
   * @param index the index of the product in this inventory
   ...
   * @return the quantity of the product in this inventory or 0 if no
   * such product is in the inventory
   */
  public int getInventoryQuantity(int productID) {
    int index = ArrayUtils.keyValuesIndexOf(inventory, PARTS,
					    PRODUCT_ID, count * PARTS,
					    productID);
    if (index >= 0) {
  ...
  /**
   * Adds a number of products of the specified product type to this
   * inventory.
   *
   * @param productID the id of the product whose quantity to change
   * @param quantity the quantity to add to this inventory
   */
   ...
   * inventory.
   *
   * Note: the quantity in this inventory status might become negative
   * if there were not enough products in the inventory.
   *
   * @param productID the id of the product whose quantity to change
  ...
  public String toString() {
    StringBuffer sb = new StringBuffer()
      .append(getTransportName()).append('[');
    for (int i = 0, n = getProductCount() * PARTS; i < n; i += PARTS) {
      if (i > 0) sb.append(',');
      sb.append(inventory[i + PRODUCT_ID]).append('=')
	.append(inventory[i + QUANTITY]);
    }
    sb.append(']');
    return sb.toString();
  }
  ...
  public void write(TransportWriter writer) {
    ...
    for (int i = 0, n = count * PARTS; i < n; i += PARTS) {
      ...
      if (quantity != 0) {
	writer.node("inventory")
	  .attr("product", inventory[i + PRODUCT_ID])
