 ...
 * Updated : $Date: 2004/06/07 08:24:40 $
 *           $Revision: 1.9 $
 */
...
public class BinaryTransportReader extends TransportReader
  implements BinaryTransport
{
  ...
  private String[] aliases = new String[10];
  ...
  private int addAlias(int pos) throws ParseException {
    ...
    pos++;
    int id = ((messageData[pos] & 0xff) << 8) + (messageData[pos + 1] & 0xff);
    pos += 2;
    int len = ((messageData[pos] & 0xff) << 8) + (messageData[pos + 1] & 0xff);
    pos += 2;
...
//     System.out.println("Adding alias " + alias + " = " + id);
    ...
    if (aliases.length <= id) {
      aliases = (String[]) ArrayUtils.setSize(aliases, id + 10);
    }
    aliases[id] = alias;
...
//     System.out.println(">> Node: " + name + " attCount = " + attNo);
  ...
  private boolean skipToNextNode() throws ParseException {
	...
	if (op == NODE || op == START_NODE) {
      ...
      // Here we need to end this node first (since it was an START_NODE)
      // and then find another node...

      // UNLESS we are in "enter node" mode, then we just need to find
      // the next node at all... FIX THIS!!!
      int level = 1;
	...
	case END_NODE:
	  level--;
	  if (nodeEntered && level == 0) return false;
	  pos++;
	  break;
	case ALIAS:
	  pos = addAlias(pos);
  ...
  private int skipNode(int pos) {
    if ((pos + 4) >= dataLen) {
      // No more complete nodes. Perhaps throw ParseException if not
      // complete node? FIX THIS!!!
      return dataLen;
    }

    int attNo = messageData[pos + 1] & 0xff;
//     System.out.println("-- skipping node: at " + pos +
// 		       " atts = " + attNo);
    pos += 4;
    int valPos = pos + 3 * attNo;
    ...
    throws ParseException
  {
    ...
    // Ensure that everything is as it was. FIX THIS!!! FIX THIS!!!
  ...
  public boolean isNode() throws ParseException {
    return currentPosition >= dataOffset;
  }
  ...
  public boolean enterNode() throws ParseException {
    if (nodeEntered || currentPosition < dataOffset) return false;
...
// 			 " pos = " + currentPosition);
  ...
  public int getAttributeCount() {
    if (currentPosition >= dataOffset) {
      return messageData[currentPosition + 1] & 0xff;
    }
    return 0;
  }
  ...
  public String getAttributeName(int index) throws ParseException {
    ...
    int pos = currentPosition + 5 + index * 3;
    int nid = ((messageData[pos] & 0xff) << 8) + (messageData[pos + 1] & 0xff);
    return getName(nid);
  }

  private Object getAttributeAsObject(int id) throws ParseException {
    int pos = currentPosition + 4;
    if (pos >= dataLen) {
      return null;
    }
  ...
  protected int[] getAttributeAsIntArray(String name,
					 boolean isRequired)
    throws ParseException {
    Integer id = (Integer) nameLookup.get(name);
	...
	throw new ParseException("Illegal value type, expected int[] got " +
				 val, 0);
      }
    }
    return null;
