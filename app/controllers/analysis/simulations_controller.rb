class Analysis::SimulationsController < Analysis::AnalysisController
  ...
  def new
    ...
    @profile_options = ThreePlayerProfile.all.collect {|s| [s.name, s.id]}
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(analysis_simulations_url) }
      format.xml  { head :ok }
    end
  end
